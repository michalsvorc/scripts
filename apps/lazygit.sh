#!/usr/bin/env bash
#
# Description: Install script for lazygit executable.
...
readonly argv0=${0##*/}
readonly repository_uri="https://api.github.com/repos/jesseduffield/lazygit/releases"
...
Download script for lazygit executable.
...
parse_download_uri() {
  printf '%s' $(\
    jq -r ".assets \
    | map(select(.name|endswith(\"${asset}\")))[0] \
